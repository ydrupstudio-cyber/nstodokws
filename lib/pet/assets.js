...
/**
 * 착용 보정 규칙 (3차 착용 수정팩).
 * 그림별로 아이템을 조금씩 옮겨 목도리가 얼굴을 덮지 않게 한다.
 * 280개 그림 원본 × 자리별 보정값이라 한 번만 받아 두고 계속 쓴다.
 */
let wearFitPromise = null;
export function loadWearFitRules() {
  if (!wearFitPromise) {
    wearFitPromise = fetch(BASE + 'pet/wear-fit-rules.json')
      .then((r) => { if (!r.ok) throw new Error('wear-fit ' + r.status); return r.json(); })
      .catch((e) => { wearFitPromise = null; throw e; });
  }
  return wearFitPromise;
}
