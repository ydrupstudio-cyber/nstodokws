...
/* 벽에 거는 물건인가. 창문·액자·선반이 여기 해당한다 */
export function isWall(item,catalog){return catalog[item.assetId]?.layer==='wall'}
export function placement(item,items,size,catalog){
  ...
  /*
    벽에 거는 물건은 보이는 두 벽에만 붙는다.
    화면의 위 꼭짓점이 (0,0) 이고, 거기서 내려오는 두 모서리가 x=0 줄과 y=0 줄이다.
    가로로 긴 물건은 그 줄을 따라 뻗어야 하므로 반대 좌표가 0 이어야 한다.
  */
  const mine=isWall(item,catalog);
  if(mine){
    if(item.x!==0&&item.y!==0)return{ok:false,reason:'벽에 거는 것은 벽 쪽에만 놓을 수 있어요.'};
    // 왼쪽 벽(x=0)에 붙으면 세로로 뻗어야 하고, 오른쪽 벽(y=0)에 붙으면 가로로 뻗어야 한다.
    // 벽을 뚫고 나오는 방향으로 놓이면 벽에서 떠 보인다 — 돌리기로 방향을 맞춘다
    if(item.x===0&&item.y!==0&&w>1)return{ok:false,reason:'돌려서 벽을 따라 놓아주세요.'};
    if(item.y===0&&item.x!==0&&h>1)return{ok:false,reason:'돌려서 벽을 따라 놓아주세요.'};
  }
  for(const other of items){
    if(other.uid===item.uid)continue;
    // 바닥에 깔리는 것은 아무와도 안 부딪힌다. 벽 물건과 바닥 물건도 서로 상관없다
    if(catalog[other.assetId].layer==='floor'||catalog[item.assetId].layer==='floor')continue;
    if(isWall(other,catalog)!==mine)continue;
    const[ow,oh]=dimensions(other,catalog);
    if(rectOverlap(r,{...other,w:ow,h:oh}))return{ok:false,reason:mine?'다른 벽장식과 겹쳐요.':'다른 가구와 겹쳐요.'};
  }
