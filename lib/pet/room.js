...
/*
  ...
  납품 팩에 창문마다 `-night` 그림이 같이 왔는데 쓰지 않고 있었다.
  저녁 8시부터 아침 6시까지는 밤 그림을 쓴다 — 당직 때 열면 창밖이
  밤인 게 이 앱답다.

  밤 그림이 없는 물건(액자·선반)은 원래 그림을 그대로 돌려준다.
  파일이 있는지 일일이 확인하지 않는다 — 창문(kind === 'window')만
  밤 그림이 온다는 것이 납품 규칙이다.
*/
export const NIGHT_FROM = 20;   // 저녁 8시
export const NIGHT_TO = 6;      // 아침 6시
...
/** 이 물건의 지금 그림 경로 */
export function sourceOf(item, catalog, night = false) {
  const a = catalog[item.assetId];
  if (!a) return '';
  const base = a.rotations?.[item.rotation || 0]?.source || a.path;
  if (!night || a.kind !== 'window') return base;
  return base.replace(/(-(?:90|180|270))?\.svg$/, (_, dir) => `-night${dir || ''}.svg`);
}
