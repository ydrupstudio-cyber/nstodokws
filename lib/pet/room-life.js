/*
  납품 원본(4차 방 팩 integration/room-life.js) 그대로다.
  앱이 더한 것은 goTo(x,y) 한 개뿐 — 바닥을 탭하면 그리로 걸어가는 길이다.
  4차에서 원본이 스스로 고친 것:
    · 숨은 뒤 계속 빼꼼 (peekDelay, 기본 0.9초). 3차 때 앱에서 덧댄 수정은 뺐다
    · 터널처럼 입구가 둘인 가구를 위한 ports
*/
...
class Controller{
 ...
 _candidate(uid,portId=null){
  const it=this.items.find(a=>a.uid===uid);if(!it)return{ok:false,reason:'missing'};let p=this._profile(it);if(!p)return{ok:false,reason:'decorative'};if(!this.canUse(it))return{ok:false,reason:'busy'};
  if(p.ports){const port=p.ports.find(a=>a.id===(portId??p.defaultPort));if(!port)return{ok:false,reason:'unknown-port'};p={...p,...port,portId:port.id};}else if(portId!==null)return{ok:false,reason:'unknown-port'};
 ...
 request(uid,{source='user',port=null}={}){
  ...
  const ports=this._profile(it).ports;if(port!==null&&(!ports||!ports.some(p=>p.id===port)))return{ok:false,reason:'unknown-port'};
  if(this.active&&['entering','using','exiting'].includes(this.phase)){
   this.pending=uid===this.active.item.uid&&(port===null||port===this.active.profile.portId)?null:{uid,source,port};if(this.phase!=='exiting')this._exit('new-request');return{ok:true,queued:!!this.pending};
  }
  const c=this._candidate(uid,port);if(!c.ok){this._emit('unavailable',{uid,reason:c.reason});return c;}
 ...
 _replan(){if(!this.active){this.path=[];this._idle();return;}const c=this._candidate(this.active.item.uid,this.active.profile.portId??null);if(!c.ok){this._release('route-blocked');this._idle();this._emit('unavailable',{reason:c.reason});return;}this.path=c.route.points;this.active.approach=c.route.end;}
 _arrive(){
 ...
 _finish(reason){const queued=this.pending;this.pending=null;this._release(reason||'finished');this._idle();if(queued)this.request(queued.uid,{source:queued.source,port:queued.port});}
 _chooseAutomatic(){
  const choices=[];for(const it of this.items){const p=this._profile(it);if(!p||this.cooldowns.get(it.uid)>this.clock||p.automatic===false)continue;const ports=p.ports?.map(p=>p.id)||[null];for(const port of ports){const c=this._candidate(it.uid,port);if(c.ok)choices.push({uid:it.uid,port,weight:(p.weight||1)/ports.length/(1+c.route.points.length*.12)});}}
  ...
  if(choices.length){let n=this.random()*choices.reduce((s,c)=>s+c.weight,0);let chosen=choices[choices.length-1];for(const c of choices){n-=c.weight;if(n<=0){chosen=c;break;}}this.request(chosen.uid,{source:'auto',port:chosen.port});}else this.idleWait=8;
 }
 tick(seconds){
 ...
 snapshot(){
  ...
  if(p?.mode==='hide'){
   ...
   if(phase==='using'){peek=t>=(p.peekDelay??.9);opacity=peek?1:0;if(peek)pose='peek';}
