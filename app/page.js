...
export default function Home() {
  ...
  useEffect(() => {
    ...
    carryOverTodos().then(() => generateRecurringForToday());
  }, [currentMember, currentDate]);

  // 어제까지의 미완료 할일을 오늘로 이월 (원래 날짜는 carried_from에 보존)
  async function carryOverTodos() {
    try {
      const { data: moved } = await supabase.rpc('carry_over_todos', { p_today: todayStr() });
      if (moved > 0) loadTasks(currentDate);
    } catch (e) {
  ...
  const normalSortFn = (a, b) => {
    // 이월된 할일이 항상 먼저
    const ac = a.carried_from ? 0 : 1;
    const bc = b.carried_from ? 0 : 1;
        ...
        <header style={styles.header}>
          <div style={styles.headerTop}>
            <div style={styles.headerLeft}>
              ...
              <button onClick={() => setShowBoards(true)} style={styles.iconBtn} title="근무표·일정">📋</button>
